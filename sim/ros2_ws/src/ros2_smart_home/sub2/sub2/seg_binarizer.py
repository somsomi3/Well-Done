...
class IMGParser(Node):
    ...
    # 로직 3. bgr 이미지의 binarization
        ...
        # 지갑, 가방, 리모컨, 키의 색상 범위를 정의합니다.
        # 필요할 경우 BGR 값을 조절하여 색상 범위를 변경해 구분 할 수 있습니다.
        # Hue: 갈색 계열
        lower_wal = np.array([10, 50, 50])   # 예: 지갑의 하한 BGR 값
        upper_wal = np.array([20, 255, 255]) # 예: 지갑의 상한 BGR 값

        lower_bp = np.array([100, 50, 50])  # 예: 가방의 하한 BGR 값
        upper_bp = np.array([130, 255, 255]) # 예: 가방의 상한 BGR 값

        lower_rc = np.array([20, 100, 100]) # 예: 리모컨의 하한 BGR 값
        upper_rc = np.array([30, 255, 255]) # 예: 리모컨의 상한 BGR 값

        # Hue: 금속성 회색
        lower_key = np.array([0, 0, 80])  # 예: 키의 하한 BGR 값
        upper_key = np.array([180, 50, 255]) # 예: 키의 상한 BGR 값

        # HSV 색 공간으로 변환 후 binarization 수행
        ...
        """
        contours_wal, _ = cv2.findContours(self.img_wal.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        contours_bp, _ = cv2.findContours(self.img_bp.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        contours_rc, _ = cv2.findContours(self.img_rc.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        contours_key, _ = cv2.findContours(self.img_key.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def find_cnt(self, contours):
        ...
        for cnt in contours:
            x, y, w, h = cv2.boundingRect(cnt)  # 바운딩 박스 좌표 계산
            if w * h > 500:  # 너무 작은 영역은 무시 (노이즈 제거)

...
class IMGParser(Node):
    ...
    def find_bbox(self):
        ...
        contours_wal, _ = cv2.findContours(
            self.img_wal.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )

        contours_bp, _ = cv2.findContours(
            self.img_bp.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )

        contours_rc, _ = cv2.findContours(
            self.img_rc.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )

        contours_key, _ = cv2.findContours(
            self.img_key.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def find_cnt(self, contours):
        ...
        for cnt in contours:
            ...
            if w * h > 500:  # 너무 작은 영역은 무시 (노이즈 제거)
                cv2.rectangle(
                    self.img_bgr, (x, y), (x + w, y + h), (0, 255, 0), thickness=2
                )

...
def non_maximum_supression(bboxes, threshold=0.5):
    ...
    # new_bboxes=[]
    bboxes = sorted(
        bboxes, key=lambda detections: detections[2] * detections[3], reverse=True
    )
    new_bboxes = []
...
class HumanDetector(Node):
    ...
    def __init__(self):
        ...
        self.subs_img = self.create_subscription(
            CompressedImage, "/image_jpeg/compressed", self.img_callback, 1
        )
    ...
    def detect_human(self, img_bgr):
        ...
        if len(rects_temp) != 0:
            # rects_temp를 리스트로 변환
            rects_temp = [
                tuple(map(int, rect)) for rect in rects_temp
            ]  # 좌표를 정수형으로 변환

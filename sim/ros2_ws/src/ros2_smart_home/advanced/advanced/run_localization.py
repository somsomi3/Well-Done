...
class Localization:
    ...
    def __init__(self, params_map, params_mcl):
        ...
        self.odom_cov = np.array(
            [
                self.odom_translation_cov,
                self.odom_translation_cov,
                self.odom_heading_cov,
            ]
        )
    ...
    def show_particle(self, particles, laser=None, gt_pose=None):
        ...
        for i in range(self.num_particle):
            ...
            # print(rand_x)
            pos_x = (
                rand_x - self.map_center[0] + (rows * self.map_resolution) / 2
            ) / self.map_resolution
            pos_y = (
                rand_y - self.map_center[1] + (cols * self.map_resolution) / 2
            ) / self.map_resolution
            # print(pos_x)
            # TODO : check rows and cols
            if (
                pos_y.astype(np.int) > 0
                and pos_x.astype(np.int) > 0
                and pos_y.astype(np.int) < rows
                and pos_x.astype(np.int) < cols
            ):
                map_bgr_with_particle.itemset(
                    pos_y.astype(np.int), pos_x.astype(np.int), 0, 0
                )
                map_bgr_with_particle.itemset(
                    pos_y.astype(np.int), pos_x.astype(np.int), 1, 255
                )
                map_bgr_with_particle.itemset(
                    pos_y.astype(np.int), pos_x.astype(np.int), 2, 0
                )
            # print(rand_x)
        ...
        if laser is not None:
            ...
            for i in range(laser_global.shape[1]):
                (l_x, l_y) = np.array([laser_global_x[i], laser_global_y[i]]).astype(
                    np.int
                )
                center = (l_x, l_y)
        ...
        map_bgr_with_particle = cv2.resize(
            map_bgr_with_particle,
            dsize=(0, 0),
    ...
    def _prediction(self, diff_pose):
        ...
        dist_noise_coeff = odom_cov[0] * np.fabs(delta_dist) + odom_cov[2] * np.fabs(
            delta_angle1 + delta_angle2
        )
        angle1_noise_coeff = odom_cov[2] * np.fabs(delta_angle1) + odom_cov[
            0
        ] * np.fabs(delta_dist)
        angle2_noise_coeff = odom_cov[2] * np.fabs(delta_angle2) + odom_cov[
            0
        ] * np.fabs(delta_dist)
    ...
    def _points_in_map(self, x_list, y_list):
        points_global_x = (
            x_list - self.map_center[0] + (self.map_size[0] * self.map_resolution) / 2
        ) / self.map_resolution
        points_global_y = (
            y_list - self.map_center[1] + (self.map_size[1] * self.map_resolution) / 2
        ) / self.map_resolution
    ...
    def _weighting(self, points):
        ...
        for i in range(self.num_particle):
            ...
            particle_pose_x, particle_pose_y = self._points_in_map(
                particle_pose[0], particle_pose[1]
            )
            ...
            is_pose_valid = (
                (particle_pose_x > 0)
                * (particle_pose_y > 0)
                * (particle_pose_x < cols)
                * (particle_pose_y < rows)
            )
            ...
            occupancy = self.map[
                particle_pose_y.astype(np.int), particle_pose_x.astype(np.int)
            ]
            if occupancy < 0.75:
                self.particles[3, i] = 0
                continue
            ...
            points_global_x, points_global_y = self._points_in_map(
            ...
            valid_idx = (
                (points_global_x > 0)
                * (points_global_y > 0)
                * (points_global_x < cols)
                * (points_global_y < rows)
            )
...
class Localizer(Node):
    ...
    def __init__(self):
        ...
        self.init_pose_sub = self.create_subscription(
            PoseWithCovarianceStamped, "/initialpose", self.init_pose_callback, 1
        )
    ...
    def turtlebot_status_callback(self, msg):
        ...
        if self.is_init_pose == True and self.is_imu == True:
            if self.is_status == False:
                ...
                self.period = (self.current_time - self.prev_time).nanoseconds / (1e9)

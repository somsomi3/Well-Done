...
from ssafy_bridge.ssafy_udp_parser import (
    erp_udp_sender,
    handControlSender,
    app_control_sender,
)
...
class ssafy_bridge(Node):
    ...
    def __init__(self):
        ...
        self.hand_sub = self.create_subscription(
            HandControl, "hand_control", self.hand_callback, 10
        )
        self.app_control_subscriber = self.create_subscription(
            Int8MultiArray, "app_control", self.app_control_callback, 10
        )

...
class erp_udp_parser:
    def __init__(self, publisher, node_or_ip, ip_or_port, port_or_type, data_type=None):

        if isinstance(node_or_ip, Node):
            self.publisher = publisher
            self.node = node_or_ip
            self.ip = ip_or_port
            self.port = port_or_type
            ...
            self.node = None
            self.ip = node_or_ip
            self.port = ip_or_port
            self.data_type = port_or_type
    ...
    def recv_udp_data(self):
        while True:
            try:
                ...
                if not self.is_sender_port:
                    self.is_sender_port = True
                    self.sender_port = sender[1]
            except Exception as e:
                print(f"[{self.data_type}] ERROR in recv_udp_data: {e}", flush=True)
        ...
        if self.data_type == "app_status":
            ...
            if header == "#Appliances$" and data_length[0] == 17:
                ...
                msg.header.stamp = (
                    ...
                    if self.node
                    else rclpy.clock.Clock().now().to_msg()
                )
                msg.data = app_data
                self.publisher.publish(msg)
        ...
        if self.data_type == "turtlebot_status":
            ...
            if header == "#Turtlebot$" and data_length[0] == 32:
                ...
                battery_charge_status = struct.unpack(
                    "B", raw_data[15 + 8 + 12 : 15 + 8 + 12 + 1]
                )[0]
                battery_percentage = struct.unpack(
                    "f", raw_data[15 + 8 + 12 + 1 : 15 + 8 + 12 + 1 + 4]
                )[0]

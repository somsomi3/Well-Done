...
class IMGPublisher(Node):
    ...
    def __init__(self):
        ...
        self.udp_parser = UDP_CAM_Parser(
            ip=params_cam_0["localIP"],
            port=params_cam_0["localPort"],
            params_cam=params_cam_0,
        )

        self.publisher_ = self.create_publisher(
            CompressedImage, "/image_jpeg/compressed", 10
        )
    ...
    def timer_callback(self):
        ...
        if len(self.udp_parser.img_byte) == 0:
            ...
            self.img_msg.header.stamp = self.get_clock().now().to_msg()  # ✅ 추가
            self.img_msg.header.frame_id = "camera"  # ✅ 추가

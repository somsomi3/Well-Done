...
def visualize_images(image_out, t_cost, boxes, classes, scores, class_names):
    ...
    for i in range(len(boxes)):
        if scores[i] >= 0.7:
            ...
            cv2.putText(
                image_out, label, (int(x1), int(y1) - 10), font, 0.5, (0, 255, 0), 1
            )
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        valid_img = (
            (xy_i[:, 0] >= 0)
            & (xy_i[:, 0] < params_cam["WIDTH"])
            & (xy_i[:, 1] >= 0)
            & (xy_i[:, 1] < params_cam["HEIGHT"])
        )
        ...
        if len(boxes_detect) != 0:
            ...
            image_process = draw_pts_img(
                image_process,
                xy_i_valid[:, 0].astype(np.int32),
                xy_i_valid[:, 1].astype(np.int32),
            )

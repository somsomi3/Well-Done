...
class Handcontrol(Node):
    ...
    def display_status(self):
        ...
        if self.is_turtlebot_status:
            print("\n🔍 현재 터틀봇 상태:")
            print(
                f"   - can_lift: {'✅ 가능' if self.turtlebot_status_msg.can_lift else '❌ 불가능'}"
            )
            print(
                f"   - can_put: {'✅ 가능' if self.turtlebot_status_msg.can_put else '❌ 불가능'}"
            )
            print(
                f"   - 현재 설정된 놓을 위치: 거리({self.put_distance}m), 높이({self.put_height}m)"
            )
    ...
    def set_put_position(self):
        ...
        try:
            ...
            print(
                f"✅ 놓을 위치 설정 완료 & 시뮬레이터에 적용! (거리: {self.put_distance}m, 높이: {self.put_height}m)"
            )
    ...
    def hand_control_put_down(self):
        ...
        if self.turtlebot_status_msg.can_put:
            print(
                f"🛠️ 오브젝트를 놓습니다. (거리: {self.put_distance}m, 높이: {self.put_height}m)"
            )
